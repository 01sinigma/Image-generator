...
class SDXLGenerator(BaseImageGenerator):
    """
    Генератор для SDXL-like моделей (SDXL Turbo, Pony, RealVisXL)
    Поддерживает LoRA и отключение цензуры
    """
    ...
    def load_model(self):
        ...
        logger.info(f"Загрузка SDXL модели: {model_name}")
        
        try:
            # Определяем тип пайплайна
            if 'turbo' in model_name.lower():
                # SDXL Turbo использует AutoPipeline
                self.pipe = AutoPipelineForText2Image.from_pretrained(
                    model_name,
                    torch_dtype=self._get_torch_dtype(),
                    variant="fp16" if self._get_torch_dtype() == torch.float16 else None,
            ...
            # Отключаем safety checker если есть
            if hasattr(self.pipe, 'safety_checker'):
                self.pipe.safety_checker = None
            
            # Оптимизация для 8GB VRAM
            device_config = self.config.get('device', {})
            if device_config.get('enable_cpu_offload', False):
                logger.info("Включение CPU offload для экономии VRAM")
                self.pipe.enable_model_cpu_offload()
            else:
            ...
            logger.error(f"Ошибка загрузки SDXL модели: {e}")
            raise
    ...
    def load_lora(self, lora_path: str, weight: float = 1.0):
        ...
        try:
            logger.info(f"Загрузка LoRA: {lora_path}")
            self.pipe.load_lora_weights(lora_path)
            self.pipe.fuse_lora(lora_scale=weight)
            logger.info(f"LoRA загружена с весом {weight}")
        except Exception as e:
    ...
    def generate(
        ...
        logger.info(f"Генерация: {width}x{height}, {num_inference_steps} шагов")
...
class InstructPix2PixGenerator(BaseImageGenerator):
    """
    Генератор для InstructPix2Pix - редактирование изображений по текстовым инструкциям
    Примеры: "сделай волосы синими", "добавь очки", "сделай ночь"
    """
    
    def load_model(self):
        """Загрузка InstructPix2Pix"""
        from diffusers import StableDiffusionInstructPix2PixPipeline
        
        model_name = self.model_config.get('name', 'timbrooks/instruct-pix2pix')
        logger.info(f"Загрузка InstructPix2Pix: {model_name}")
        ...
        logger.info("InstructPix2Pix загружена")
    ...
    def edit(
        ...
        Args:
            ...
            prompt: Инструкция (напр. "сделай волосы синими")
            num_inference_steps: Количество шагов
            guidance_scale: Сила следования промпту
            image_guidance_scale: Сила сохранения оригинала (выше = меньше изменений)
        """
        if self.pipe is None:
            raise RuntimeError("Модель не загружена")
        ...
        logger.info(f"Редактирование: '{prompt}'")
        ...
        if save_path:
            self._save_image(edited_image, save_path)
    ...
    def generate(self, *args, **kwargs):
        """InstructPix2Pix не поддерживает генерацию с нуля"""
        raise NotImplementedError("InstructPix2Pix только редактирует изображения. Используйте edit()")


class SDXLInpaintingGenerator(BaseImageGenerator):
    """
    Генератор для SDXL Inpainting - закрашивание/замена частей изображения
    Требует маску: белое = заменить, чёрное = сохранить
    """
    
    def load_model(self):
        """Загрузка SDXL Inpainting"""
        from diffusers import StableDiffusionXLInpaintPipeline
        
        model_name = self.model_config.get('name', 'diffusers/stable-diffusion-xl-1.0-inpainting-0.1')
        logger.info(f"Загрузка SDXL Inpainting: {model_name}")
        ...
        logger.info("SDXL Inpainting загружена")
    ...
    def inpaint(
        ...
        """
        Закрашивание/замена части изображения
        
        Args:
            image: Входное изображение
            mask: Маска (белое = заменить, чёрное = сохранить)
            prompt: Что нарисовать в замаскированной области
            negative_prompt: Что НЕ рисовать
            strength: Сила изменений (0-1)
        """
        if self.pipe is None:
            raise RuntimeError("Модель не загружена")
        ...
        logger.info(f"Inpainting: '{prompt}'")
        ...
        if save_path:
            self._save_image(inpainted_image, save_path)
    ...
    def generate(self, *args, **kwargs):
        """SDXL Inpainting требует маску"""
        raise NotImplementedError("SDXL Inpainting требует изображение и маску. Используйте inpaint()")
...
class ModelFactory:
    ...
    MODEL_TYPES = {
        ...
        'sdxl-turbo': SDXLGenerator,
        'pony-diffusion': SDXLGenerator,
        'realvis-xl': SDXLGenerator,
    ...
    def create_generator(model_type: str, config_path: str = "config.yaml") -> BaseImageGenerator:
        ...
        if model_class is None:
            # Попробуем определить тип по конфигурации
            if 'sdxl' in model_type.lower() or 'pony' in model_type.lower() or 'realvis' in model_type.lower():
                model_class = SDXLGenerator
            else:
